...
function UpdateProject({ isEnLang = false, id = "#" }) {
  ...
  const fetchProject = async () => {
    await fetchData(`/api/project/${id}`, setProject, setLoading, setError);
  };
  useEffect(() => {
    fetchProject();
  }, []);
  useEffect(() => {
    if (project) {
      setFa_title(project.fa_title);
      setFa_employer(project.fa_employer);
      setFa_startDateYear(project.fa_startDate.year);
      setFa_startDateMonth(project.fa_startDate.month);
      setFa_duration(project.fa_duration);
      setFa_description(project.fa_description);
      setEn_title(project.en_title);
      setEn_employer(project.en_employer);
      setEn_startDateYear(project.en_startDate.year);
      setEn_startDateMonth(project.en_startDate.month);
      setEn_duration(project.en_duration);
      setEn_description(project.en_description);
      setImages([]);
      project.images &&
        project.images.forEach((image) => {
          setImages((prev) => [...prev, { file: image, preview: image }]);
        });
    }
  }, [project]);
        ...
        .then((response) => {
          Swal.fire({
            ...
            didClose: () => {
              ...
              clearData();
              router.replace(
                `/${isEnLang ? "en" : "fa"}/projects/project?id=${id}`
              );
            },
          });
        })
        .catch((error) => {
          console.log(" (update project PUT) error:", error);
          Swal.fire({
            toast: true,
            position: "top-end",
            icon: "Error",
  ...
  return (
    <>
      {project ? (
        <div className=" container mx-auto flex flex-col pt-8 items-center">
          <SectionTittle
            tittle={isEnLang ? "Project Update" : "ویرایش پروژه"}
          />
