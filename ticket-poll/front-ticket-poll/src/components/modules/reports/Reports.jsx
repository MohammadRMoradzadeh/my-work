...
const Reports = () => {
  ...
  const exportToExcel = async () => {
    try {
      ...
      // اصلاح مسیر API برای خروجی اکسل
      const response = await fetch(`http://localhost:8000/api/reports/api/export-ticket-reports/?${params.toString()}`, {
        headers: {
          'Authorization': `Token ${token}`
        }
      });
  ...
  const formatTime = (value, type) => {
    if (!value || value === "اطلاعاتی موجود نیست") return "—";
    if (type === 'resolution' || type === 'response') {
      const hoursMatch = value.match(/(\d+) ساعت/);
      const minutesMatch = value.match(/(\d+) دقیقه/);
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div>
              ...
              >
                ...
                <option value="superuser">سوپر کاربر</option>
              </select>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
          ...
          <div className="bg-indigo-50 p-4 rounded-lg border border-indigo-200">
            ...
            <div className="text-2xl font-bold text-indigo-700">
              {formatTime(reportsData.tickets.total_response_time, 'response')}
            </div>
          </div>
        </div>
      </div>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {reportsData.tickets_details.map((ticket) => (
                <tr key={ticket.id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{getStatusInPersian(ticket.status)}</td>
