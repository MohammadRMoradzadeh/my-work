...
const AddSurvey = () => {
  ...
  useEffect(() => {
    ...
    const fetchUser = async () => {
      try {
        ...
        if (
          !(
            (data.user_type === "employee" && data.can_create_poll) ||
  ...
  useEffect(() => {
    ...
    const selectedDepartment = departments.find(
      (d) => d.id === parseInt(form.department)
    );
    const isStudentsDepartment =
      selectedDepartment && selectedDepartment.name === "دانشجویان";
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setMessage(
          `ثبت نظرسنجی موفق نبود. جزئیات خطا: ${JSON.stringify(errorData)}`
        );
        setMessageType("error");
  ...
  if (
    !user ||
    !(
      (user.user_type === "employee" && user.can_create_poll) ||
      ...
      <div className="min-h-screen flex items-center justify-center font-Estedad">
        <div className="text-center">
          ...
          <p className="text-gray-600 mt-2">
            شما مجاز به ایجاد نظرسنجی نیستید.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="container mx-auto px-4 py-4 sm:px-6 sm:py-6 max-w-2xl">
      ...
      {message && (
        <div
          className={`p-3 rounded-md mb-4 text-center font-Estedad ${
            messageType === "success"
