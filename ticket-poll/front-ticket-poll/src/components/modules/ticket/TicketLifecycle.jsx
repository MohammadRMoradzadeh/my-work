...
const TicketLifecycle = () => {
  ...
  useEffect(() => {
    const fetchLifecycleData = async () => {
      try {
        const response = await fetch(
          `http://localhost:8000/api/reports/api/ticket-lifecycle/${ticketId}/`,
          {
            headers: {
              Authorization: `Token ${token}`,
            },
  ...
  if (
    ...
    !userPermissions.view_report_tickets
  ) {
    return (
      ...
      <div className="flex justify-center items-center h-64">
        <div className="flex flex-col items-center">
          ...
          <div className="text-xl font-semibold">
            در حال بارگذاری چرخه حیات تیکت...
          </div>
        </div>
      </div>
    );
      ...
      <div className="bg-white p-6 rounded-lg shadow-md border border-gray-200">
        <div className="mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <span className="font-semibold">شماره تیکت:</span>{" "}
              {lifecycleData.ticket.id}
            </div>
            <div>
              <span className="font-semibold">وضعیت:</span>{" "}
              {lifecycleData.ticket.status}
            </div>
            <div>
              <span className="font-semibold">تاریخ ایجاد:</span>{" "}
              {formatDateTime(lifecycleData.ticket.created_at)}
            </div>
            <div>
              <span className="font-semibold">کاربر:</span>{" "}
              {lifecycleData.ticket.user}
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          {lifecycleData.lifecycle_events &&
          lifecycleData.lifecycle_events.length > 0 ? (
            <div className="space-y-4">
              {lifecycleData.lifecycle_events.map((event, index) => {
                const isReferredEvent = event.action === "referred";
                const responseTime = isReferredEvent
                  ? calculateResponseTime(lifecycleData.lifecycle_events, index)
                  : null;
                ...
                return (
                  ...
                  >
                    <div className="flex justify-between">
                      <h3
                        className={`font-bold ${
                          isReferredEvent ? "text-green-700" : ""
                        }`}
                      >
                    ...
                    {event.description && (
                      <p
                        className={`mt-1 ${
                          isReferredEvent ? "text-green-600" : "text-gray-600"
                        }`}
                      >
                        {event.description}
                      </p>
                    ...
                    {event.user && (
                      <p
                        className={`text-sm mt-1 ${
                          isReferredEvent ? "text-green-500" : "text-gray-500"
                        }`}
                      >
                        توسط: {event.user}
                      </p>
            ...
            <p className="text-gray-500">
              هیچ رویدادی برای این تیکت ثبت نشده است.
            </p>
          )}
        </div>
      </div>
